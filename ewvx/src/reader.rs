...
fn validate_version(input: &str) -> Result<()> {
    let tag_start = input.find("<video").context("Missing <video> root element")?;
    let tag_end = tag_start + input[tag_start..].find('>').context("Malformed <video> tag")?;
    let tag = &input[tag_start..tag_end];

    let version = read_attribute(tag, "version")
        .context("Missing version attribute on <video>")?;
    if version != "2.0" {
...
fn read_track(part: &str) -> Result<EwvxTrack> {
    ...
    let info_end = part.find("</track-info>").context("Missing </track-info>")?;
...
fn read_segment(part: &str) -> Result<EwvxSegment> {
    let index = read_attribute(part, "index")?
        .parse::<usize>().context("Invalid segment index")?;
    let timestamp = read_attribute(part, "timestamp")?
        .parse::<f64>().context("Invalid segment timestamp")?;
    let sample_offset = read_attribute(part, "sample-offset")?
        .parse::<u64>().context("Invalid segment sample-offset")?;
    let sample_count = read_attribute(part, "sample-count")?
        .parse::<u32>().context("Invalid segment sample-count")?;
